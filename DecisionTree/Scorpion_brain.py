"""
scorpion_bot.py  —  Block-First Scorpion

Same architecture as Kollector v12:
  - DEFAULT = hold back (blocking)
  - Threat detection via HP drop, approach velocity, distance, raw input
  - Only attacks when safe window confirmed
  - After every attack, returns to blocking
...
COOLDOWN = {
    ...
    "21_hp_spear":     18, "21_hp_spin":     18, "21_hp_kill":   20,
    "b14_hp_spear":    18, "b14_hp_spin":    16,
    "f32_hp_spear":    18, "f42_hp_spear":   18, "f42_hp_spin":  16,
    "11_hp_spear":     18, "d2_aa":          14, "d2_aa_spin":   14,
    # Quick cancels
    "21_spear":        10, "212_spear":      12, "212_spin":     12,
    "f32_spear":       10, "f32_spin":       10,
    "f42_spear":       10, "b14_spear":      10,
    "b143":            10,
    # Raw specials
    "raw_spear":        8, "raw_hp":          8, "raw_hp_amp":   16,
    "death_spin":       8, "burning_spear":   6,
    # Pokes
    "d1": 3, "d2": 5, "d3": 3, "d4": 3,
    "1": 3, "2": 4, "3": 3, "4": 4,
    # Directional normals
...
# ═══════════════════════════════════════════════════════════════════════
# THREAT DETECTOR (identical to Kollector v12)
# ═══════════════════════════════════════════════════════════════════════
...
class ThreatDetector:
    ...
    def update(self, p1, p2):
        ...
        opp_hp = p2['hp']
        if opp_hp < self.last_opp_hp - 0.002: self.frames_since_opp_dmg = 0
        else:
    ...
    def we_took_damage_recently(self, frames=6): return self.frames_since_our_dmg <= frames
    def we_are_being_combod(self): return self.consecutive_dmg_frames >= 2
    def our_hp_is_stable(self, frames=20): return self.frames_since_our_dmg >= frames
    ...
    def should_block(self, dist):
        if self.we_took_damage_recently(frames=6):
            return True, self.raw_input_low(), "damage"
        if self.we_are_being_combod():
            return True, self.raw_input_low(), "combo"
        if self.opponent_rushing() and dist < 450:
            return True, self.raw_input_low(), "rush"
        if self.opponent_grounded_approaching() and dist < 350:
            return True, self.raw_input_low(), "approach"
        if self.raw_input_attack() and dist < 400:
            return True, self.raw_input_low(), "input"
...
class ScorpionBot:
    ...
    def decide(self, p1, p2):
        ...
        # P3: Cooldown
        if self.cd > 0:
            self.cd -= 1; self._default_guard(); return "GUARDING"

        # P4: Safe?
        if not self.threat.is_safe_to_attack():
            self._default_guard(); return "GUARDING [not safe]"
    ...
    def _off_mid(self, p1, p2, dist):
        ...
        cuts = np.cumsum([
            0.14,   # 2,1 ~ DB3A full combo
            lw,     # F4,2 low starter combo
            mw,     # B1,4 ~ DB3A combo
            0.09,   # F3,2 ~ DB3A advancing combo
            0.08,   # B1,4,3 safe string
            0.07,   # 2,1,2 ~ BF1 spear
            0.06,   # F3,2 ~ BF1 quick
            0.05,   # Raw teleport amp combo
            0.04,   # F3 advance
            0.04,   # B1 poke
            0.04,   # B2 reach
            0.03,   # D3 low
...
if __name__ == "__main__":
    print("=" * 70)
    print("  SCORPION BOT — Block-First Architecture")
    print("=" * 70)
    print()
    print("  GET OVER HERE!")
    print("  Spear confirms | Hell Port pressure | Low/mid mixups")
